...
5. On TTL expiry or unenroll → Creddy archives the key
...
Creddy detects that Anthropic requires browser authentication and walks you through:

```
Adding backend 'anthropic'...

Anthropic requires browser authentication to create API keys.
On your laptop run:
...
2. You log in normally (Google, email, 2FA — whatever you use)
...
Backend 'anthropic' added. Session expires in ~30 days.
```

### 3. Session Renewal

Sessions last approximately 30 days. When expired, Creddy prompts during `creddy get`:

```bash
$ creddy get anthropic
⚠️ Anthropic session expired. Re-authenticate? [Y/n]

On your laptop run:
  npx @creddy/auth http://your-server:8400/v1/auth/anthropic/xyz789
...
```bash
creddy backend configure anthropic
```
...
```bash
creddy backend add anthropic --manual
```
...
```
Adding backend 'anthropic' (manual mode)...
