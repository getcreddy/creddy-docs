...
1. **Client generates a keypair** — Ed25519 keys are generated locally
2. **Client requests enrollment** — Sends public key to server
3. **Admin approves** — Via CLI, Slack, or other integration
4. **Client is activated** — All requests are now signed with the private key

```
┌─────────────┐                           ┌─────────────┐
│   Client    │  1. Generate keypair      │             │
│             │  2. Send public key ────▶ │   Creddy    │
│             │                           │   Server    │
│             │  (pending approval)       │             │
└─────────────┘                           └─────────────┘
                                                 │
                                          ...
                                          └─────────────┘
                                                 ...
                                                 ▼
┌─────────────┐                           ┌─────────────┐
│   Client    │  5. Enrollment complete   │   Creddy    │
│             │ ◀──────────────────────── │   Server    │
│             │                           │             │
│             │  6. Signed requests ────▶ │             │
└─────────────┘                           └─────────────┘
...
⣾ Waiting for approval... (4:32 remaining)

✓ Approved by admin@example.com
✓ Client registered: cli_xyz789
✓ Credentials saved to ~/.creddy/
...
```
~/.creddy/
├── config.yaml      # Server URL, client ID
├── client.key       # Ed25519 private key (mode 0600)
